#!/usr/bin/env bash
# Run ~50 diverse Euclid 1′ paper-candidate sims (Trilogy RGB).
#
# Usage:
#   ./scripts/local/run_paper_select_50.sh
#   ./scripts/local/run_paper_select_50.sh --fg

set -euo pipefail
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

CONFIG="configs/euclid_paper_select_50.yaml"
OUT_DIR="outputs/euclid_paper_select_50"
N_LENSES=50
N_FIELD_MAX=80
SEED=314159
FOREGROUND=0
...
echo "[RUN] out=$OUT_DIR n=$N_LENSES seed=$SEED (Trilogy RGB)"
